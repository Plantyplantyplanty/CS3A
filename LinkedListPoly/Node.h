...
template <typename T>
node<T>* _insertSorted(node<T>*& head, T item, bool ascending = true, bool unique = false);

template <typename T>
node<T>* _insertSorted_and_add(node<T>*& head, T item, bool ascending = true); //insert or add if a dup
...
template <typename T>
node<T>* _insertSorted(node<T>*& head, T item, bool ascending, bool unique) {
    ...
    if (head != nullptr) {
        ...
        if (follower == nullptr) {
            ...
            if ((follower->_item == item) && (unique == true)) {
                follower->_item = item;
                to_return = follower;
            }
            else {
...
template <typename T>
node<T>* _insertSorted_and_add(node<T>*& head, T item, bool ascending) {
    node<T>* to_return = nullptr;
    ...
    if (head != nullptr) {
        ...
        else if (follower->_item == item) {
            follower->_item = follower->_item + item;
            to_return = follower;
        }
        else {
            to_return = _insertAfter(head, follower, item);
